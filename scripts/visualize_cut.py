...
def main():
    parser = argparse.ArgumentParser(description="Visualize CUT translation")
    parser.add_argument(
        "--run_id", type=str, default="loop48_cut", help="Run ID of the trained CUT"
    )
    parser.add_argument(
    ...
    # StandardTrainer (and thus CUTTrainer) saves model_state_dict which is the generator
    gen_state = state.get("model_state_dict", state)
    # Remove 'module.' if present
    gen_state = {k.replace("module.", ""): v for k, v in gen_state.items()}

    input_shape = (3, 256, 256)
    # CUT uses 9 residual blocks as per CUTTrainer implementation
    generator = GeneratorResNet(input_shape, num_residual_blocks=9).to(device)

    # We might have extra keys from the F head or D if saved in the same dict,
    # but model_state_dict should only be G.
    try:
    ...
    # 2. Load Dataset (Domain A: Uncovered IR)
    ...
    for i in range(args.num_samples):
        ...
        # Handle 3-channel (replicated) input
