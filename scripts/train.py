...
def train():
    ...
    parser.add_argument("--uda", action="store_true", help="Enable Unsupervised Domain Adaptation")
    parser.add_argument("--lambda_adv", type=float, default=None, help="UDA Adversarial weight")
    ...
    if args.lambda_anatomical is not None: config["training"]["lambda_anatomical"] = args.lambda_anatomical
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=config["training"].get("batch_size", 16),
        shuffle=(train_sampler is None), sampler=train_sampler,
        collate_fn=collate_skip_none, num_workers=4 if os.name != "nt" else 0
    )
    val_loader = torch.utils.data.DataLoader(
        val_dataset, batch_size=config["training"].get("batch_size", 16),
        shuffle=False, collate_fn=collate_skip_none, num_workers=4 if os.name != "nt" else 0
    )
    ...
    if args.resume:
        ...
        if ckpt_files:
            ...
            if rank == 0: print(f"Resuming from: {latest_ckpt}")
            ...
            if "discriminator_state_dict" in state and hasattr(trainer, "discriminator"):
