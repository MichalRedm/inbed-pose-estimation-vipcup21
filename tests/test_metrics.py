...
def test_pck_vis_masking():
    ...
    # Correct: 0, 1 (count=2)
    # PCK = 2/4 = 0.5
    mean_pck, per_joint_pck = compute_pck(preds, gts, visibility=visibility, threshold=0.5)
    
    assert mean_pck == 0.5
    assert per_joint_pck[0] == 1.0
    assert per_joint_pck[1] == 1.0
    assert per_joint_pck[2] == 0.0 # Missing joint count is 1 in pose.py's per_joint_pck due to clamp, but masked in mean
