...
def test_advanced_cover_augmentation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert len(adv_cover.reference_bank) == 1, (
            "Should have loaded 1 reference image"
        )
        ...
        assert not np.all(img_aug_np == 128), (
            "Image should have been modified by cover styling"
        )
        ...
        assert np.all(img_aug_np[:30, :] == 128), (
            "Top of image (head region) should remain untouched"
        )
