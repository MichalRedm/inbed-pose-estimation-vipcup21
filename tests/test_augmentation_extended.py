...
def test_cutout_augmentation():
    ...
    assert np.array_equal(np.array(img_aug_off), np.array(img)), "Probability 0 should make no change"
...
def test_intensity_jitter():
    ...
    assert np.all(img_np >= 0) and np.all(img_np <= 255), "Values must remain in bounds [0, 255]"
