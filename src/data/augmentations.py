...
class CycleGANAugmentation:
    ...
    METADATA = {
        ...
        "order": 40,
        "parameters": [
            {"name": "probability", "type": "float", "default": 0.5, "min": 0.0, "max": 1.0}
        ],
    }

    def __init__(self, probability: float = 0.5, checkpoint_path: str = "models/cyclegan_gen_A2B.pth"):
        self.probability = probability
        ...
        if self.enabled:
            ...
            try:
                ...
                self.generator = GeneratorResNet((3, 256, 256)) # Assuming RGB/Replicated channels
                if os.path.exists(checkpoint_path):
                    ...
                    print(f"Warning: CycleGAN checkpoint {checkpoint_path} not found. Augmentation will be a no-op.")
                    self.enabled = False
            except ImportError:
                print("Failed to import GeneratorResNet.")
                self.enabled = False

    def __call__(self, img: Union[Image.Image, torch.Tensor], **kwargs) -> Union[Image.Image, torch.Tensor]:
        if not self.enabled or torch.rand(1).item() > self.probability:
            return img
...
class DataAugmenter:
    ...
    def __init__(self, config: Optional[dict] = None, is_training: bool = True):
        ...
        self.cyclegan = CycleGANAugmentation(
            ...
            checkpoint_path=self.config.get("cyclegan_path", "models/cyclegan_gen_A2B.pth")
        )
    ...
    def __call__(
        ...
        # Apply cyclegan before thermal augmenter so thermal can add extra physical properties if both are on
