...
class JointSpatialChannelAttention(nn.Module):
    """
    Joint-Symmetric Spatial-Channel Attention (JSSCA-v4 Post-Processor).
    Uses a highly focused 14-joint attention bottleneck (sequence length = 14)
    to eliminate visual token dilution. Preserves fine-grained sub-pixel coordinate
    details via a U-Net multi-scale skip routing scheme connected directly to a
    progressive deconvolutional decoder.
    """
    ...
    def __init__(self, num_joints=14, embed_dim=256, num_heads=4):
        ...
        # Processes each joint's (1, 64, 64) heatmap independently
    ...
    def forward(self, heatmaps):
        ...
        h4 = self.enc_down3(h3)     # (B*J, 64, 8, 8)
        ...
        # 6. Progressive upsampling with U-Net multi-scale skip routing
