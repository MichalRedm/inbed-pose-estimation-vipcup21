...
class CloudflaredProxy:
    ...
    def start(self, timeout: float = 15.0):
        cmd = [
            ...
            '--listener', f'127.0.0.1:{self.local_port}',
        ]
...
class GPUSession:
    ...
    def __init__(self, config: BackendConfig):
        if not _HAS_PARAMIKO:
            raise ImportError(
                'paramiko and scp are required: pip install paramiko scp'
            )
        self.config = config
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--json', default='gpu_connection.json',
    ...
    parser.add_argument(
        '--cmd', default='nvidia-smi && python -c "import torch; print(torch.cuda.get_device_name(0))"',
