...
class UDATrainer(BaseTrainer):
    ...
    def _train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        current_epoch = self.total_steps / self.num_batches_per_epoch if hasattr(self, 'num_batches_per_epoch') else 0
        alpha = min(1.0, current_epoch / self.warmup_epochs) if self.warmup_epochs > 0 else 1.0
        
        # HRNet forward with feature return
        # model expects (2B, C, H, W)
        outputs_combined, features_combined = self.model(input_combined, return_features=True)
